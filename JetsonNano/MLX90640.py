...
class MLX90640:
    ...
    def GetVideoStream(self, fps=8):
        """
        Captures thermal video stream, applies fixed colormap, detects normal temps,
        fever, and potential fire based on max temperature, and displays alerts.
        """
        try:
            # Attempt to get current refresh rate (assuming it returns the code or fps)
            # refresh_rate = self.GetRefreshRate()
            pass # Placeholder - actual check/set logic might depend on exact Get/SetRefreshRate behavior
        except Exception as e:
            print(f"Warning: Could not get initial refresh rate: {e}")
            # Continue with default or attempted setting

        # --- Set Desired Frame Rate ---
        target_rate_code = -1
        if fps == 1: target_rate_code = 0b001
        elif fps == 2: target_rate_code = 0b010
        elif fps == 4: target_rate_code = 0b011
        elif fps == 8: target_rate_code = 0b100
        elif fps == 16: target_rate_code = 0b101
        elif fps == 32: target_rate_code = 0b110
        elif fps == 64: target_rate_code = 0b111
        else:
            print(f'Error: Invalid fps setting ({fps}). Valid are 1, 2, 4, 8, 16, 32, 64.')
            return None, None # Indicate failure

        if target_rate_code != -1:
            try:
                print(f"Attempting to set refresh rate code to: {target_rate_code:03b}")
                self.SetRefreshRate(target_rate_code)
                time.sleep(0.1) # Allow setting to take effect
            except Exception as e:
                print(f"Error setting refresh rate: {e}. Continuing anyway...")
                # Decide if fatal or just a warning

        # --- Configuration ---
        fever_threshold = 37.8  # Celsius - Threshold for fever warning
        fire_threshold = 80.0   # Celsius - Threshold for fire warning (ADJUST THIS!)
        min_temp = 20.0         # Min temp for the color scale (blue)
        max_temp = 38.5         # Max temp for the color scale (red) - keeps human range visible
        scale_factor = 20       # Image resize factor for display
        box_scale_factor = 1.5  # Hotspot box size relative to scaled pixel

        print(f"Starting video stream. Fever > {fever_threshold}C, Fire > {fire_threshold}C. Color scale: {min_temp}-{max_temp}C.")

        while True:
            try:
                # --- Get Frame Data ---
                time.sleep(0.05)
                frame_data = self.GetFrameData() # Expects [subpage0_temps, subpage1_temps] or None
                if frame_data is None or len(frame_data) != 2:
                    print("Warning: Failed to get valid frame data. Skipping frame.")
                    time.sleep(1.0 / fps if fps > 0 else 0.1)
                    continue

                # --- Process Frame: Average Subpages & Flip ---
                image = np.zeros((24, 32), np.float32)
                for y in range(24):
                    for x in range(32):
                        row_idx = 23 - y # Flip vertically
                        # Basic error check for temp values if needed, though CalculateTo should handle extremes
                        val1 = frame_data[0][32 * row_idx + x]
                        val2 = frame_data[1][32 * row_idx + x]
                        image[y][x] = (val1 + val2) / 2.0

                # --- Smooth and Find Hottest Spot ---
                image_smooth = cv2.GaussianBlur(image, (5, 5), 0)
                max_detected_temp = np.max(image_smooth)
                max_index_flat = np.argmax(image_smooth)
                max_coords_yx = np.unravel_index(max_index_flat, image_smooth.shape) # (y, x)

                # --- Determine Alert Level and Message ---
                alert_level = 0 # 0: Normal, 1: Fever, 2: Fire
                alert_message = ""
                display_coords = f"({max_coords_yx[1]}, {max_coords_yx[0]})" # (x, y)

                if max_detected_temp > fire_threshold:
                    alert_level = 2
                    alert_message = f"FIRE DETECTED: {max_detected_temp:.1f}C at {display_coords}"
                    print(alert_message) # Log fire alerts
                elif max_detected_temp > fever_threshold:
                    alert_level = 1
                    alert_message = f"FEVER DETECTED: {max_detected_temp:.1f}C at {display_coords}"
                    # print(alert_message) # Optional log for fever
                else:
                    alert_level = 0
                    alert_message = f"Max Temp: {max_detected_temp:.1f}C" # Show normal max temp

                # --- Apply Color Scaling (Fixed Human Range) ---
                # Clip to the min/max chosen for color visualization, NOT the detection thresholds
                image_clipped = np.clip(image_smooth, min_temp, max_temp)
                # Scale this clipped range to 0-255
                image_scaled = ((image_clipped - min_temp) / (max_temp - min_temp)) * 255.0
                image_uint8 = image_scaled.astype(np.uint8)

                # --- Apply Colormap and Resize ---
                image_color = cv2.applyColorMap(image_uint8, cv2.COLORMAP_JET)
                image_display = cv2.resize(image_color, (32 * scale_factor, 24 * scale_factor), interpolation=cv2.INTER_LINEAR)

                # --- Prepare Visual Alerts ---
                text_color = (255, 255, 255) # Default: White (BGR)
                border_color = None          # No border by default
                draw_hotspot_box = False     # No hotspot box by default
                font_scale = 0.6
                font_thickness = 1

                if alert_level == 2: # Fire Alert
                    text_color = (0, 255, 255)   # Yellow text
                    border_color = (0, 255, 255) # Yellow border
                    draw_hotspot_box = True
                    font_scale = 0.9
                    font_thickness = 3
                elif alert_level == 1: # Fever Alert
                    text_color = (0, 0, 255)     # Red text
                    border_color = (0, 0, 255)   # Red border
                    draw_hotspot_box = True
                    font_scale = 0.7
                    font_thickness = 2
                # else: Normal - uses defaults (white text, no border/box)

                # --- Draw Alerts on Display Image ---
                # Draw Text (always shows status)
                cv2.putText(image_display, alert_message, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, font_scale, text_color, font_thickness, cv2.LINE_AA)

                # Draw Border (if fever or fire)
                if border_color:
                    cv2.rectangle(image_display, (0, 0), (image_display.shape[1] - 1, image_display.shape[0] - 1), border_color, 5)

                # Draw Hotspot Box (if fever or fire)
                if draw_hotspot_box:
                    center_x = int(max_coords_yx[1] * scale_factor + scale_factor / 2)
                    center_y = int(max_coords_yx[0] * scale_factor + scale_factor / 2)
                    box_size = int(scale_factor * box_scale_factor)
                    half_box = box_size // 2
                    pt1 = (max(0, center_x - half_box), max(0, center_y - half_box))
                    pt2 = (min(image_display.shape[1] - 1, center_x + half_box), min(image_display.shape[0] - 1, center_y + half_box))
                    cv2.rectangle(image_display, pt1, pt2, (255, 255, 0), 2) # Cyan box

                # --- Display Frame ---
                cv2.imshow('MediGuard Thermal Screening', image_display)

                # --- Handle User Input ---
                key = cv2.waitKey(1) # Essential for imshow to update
                if key & 0xFF == ord('q'):
                    print("Quit key pressed. Exiting video stream.")
                    break
                # Add other key controls here if needed ('p' for pause, etc.)

            except KeyboardInterrupt:
                print("Ctrl+C detected. Exiting video stream.")
                break
            except Exception as e:
                # Catch other potential errors during frame processing
                print(f"Error during video loop: {e}")
                # Consider adding a short pause or break depending on error severity
        ...
        # --- Return Last Frame (Optional) ---
        # Check if 'image' and 'image_color' were successfully created in the loop
        if 'image' in locals() and 'image_color' in locals():
             # Return the last raw averaged frame and the last colored frame
             return image, image_color
        else:
             # Return None if the loop didn't run or failed early
             return None, None
